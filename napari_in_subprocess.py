#!/usr/bin/python3
# ---------------------------------------------------------------------------
# Part of `tools-patched`: a patched, vendored copy of two modules from
# Andrew G. York's `tools` repo (https://github.com/AndrewGYork/tools), GPL-2.0.
#
# File: napari_in_subprocess.py
# Original authors: Nathaniel H. Thayer and Andrew G. York.
# Upstream baseline (unmodified) this patch derives from:
#   https://github.com/AndrewGYork/tools/blob/412171c3fa5e5cc8997f36ef3941d7a2f1f40686/napari_in_subprocess.py
#
# MODIFIED 2026-08-03 by amsikking:
#   * Force a Qt6 binding (PySide6, else PyQt6) inside the napari child process
#     via QT_API -- fixes corrupt/triangular/black canvas on recent NVIDIA
#     drivers (observed on 595.95).
#   * Replace the removed `napari.gui_qt()` context manager with `napari.run()`,
#     so this works on napari 0.4.19 through 0.8+.
#   * Defer the `napari`/`qtpy` imports from module-import time into the child
#     process, so importing this module never disturbs the parent's Qt binding.
# See PATCHES.md for the full change log. This modified file remains GPL-2.0.
# ---------------------------------------------------------------------------
...
# NOTE: We deliberately do NOT `import napari` (or qtpy) at module import time.
# Those imports -- and the choice of Qt binding -- happen only inside the napari
# child process (see `_napari_child_loop`). This keeps the parent process's Qt
# configuration completely untouched, so importing this module has no effect on
# any Qt GUI the surrounding application may be running.
...
class _NapariDisplay:
    ...
    """
    def __init__(self):
        import napari # Imported here so it only loads in the child process.
...
def _select_qt_binding():
    """Make qtpy/napari use a Qt6 binding (PySide6 or PyQt6) in THIS process.

    Modern napari (>=0.6) has dropped support for the old PyQt5/PySide2
    bindings; using them renders a black (or otherwise broken) canvas. We set
    the QT_API environment variable *before* qtpy is ever imported so qtpy
    picks a working Qt6 binding.

    This runs inside the napari child process only, so it never changes the
    Qt binding used by the parent process / surrounding application.
    """
    import os, importlib.util
    qt6 = {'pyside6': 'PySide6', 'pyqt6': 'PyQt6'} # PySide6 preferred.
    current = os.environ.get('QT_API', '').lower()
    # If a Qt6 binding was already selected and is importable, respect it.
    if current in qt6 and importlib.util.find_spec(qt6[current]) is not None:
        return
    # Otherwise force the first available Qt6 binding.
    for api, module_name in qt6.items():
        if importlib.util.find_spec(module_name) is not None:
            os.environ['QT_API'] = api
            return
    # No Qt6 binding is installed. Leave QT_API alone and let napari raise a
    # clear error; the fix is to `pip install PySide6` (or PyQt6).
...
def _napari_child_loop(child_pipe, initializer, initargs, initkwargs,
    ...
    This is the modern replacement for the old implementation, which used the
    `with napari.gui_qt(): ...` context manager. `napari.gui_qt()` was removed
    in napari 0.5, so we now drive the Qt event loop directly with
    `napari.run()` (its __exit__ used to do exactly this).
    """
    # Choose a Qt6 binding before importing qtpy/napari (child process only).
    _select_qt_binding()
    import napari
    ...
    while state['keep_launching_napari']:
        ...
        # Make sure a QApplication exists before we build the viewer/timer.
        _ensure_qapp()
        ...
        # Run Qt's event loop until all napari windows are closed. This blocks
        # here (just like the old `gui_qt()` context manager's __exit__ did),
        # processing `communicate()` on the QTimer the whole time.
        napari.run()
